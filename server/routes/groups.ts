...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      // Broadcast to every member's socket so open panels/room lists
      // reflect the edit live. Receivers re-derive their own
      // currentUserId/currentUserRole from the members list, since the
      // enriched room is built from the acting admin's perspective.
...
// ---------- PATCH /api/groups/:roomId/participants/:userId ----------
// Change a participant's role (currently only used to promote a member
// to admin). Admin-only; enforced in the service.
router.patch(
  '/groups/:roomId/participants/:userId',
  writeLimiter,
  async (req, res) => {
    try {
      const adminId = req.userId as string;
      const adminJWT = req.userJWT as string;
      const { roomId, userId } = req.params as {
        roomId: string;
        userId: string;
      };
      ...
      const room = await setGroupMemberRole({
        roomId,
        adminId,
        adminJWT,
        targetUserId: userId,
      ...
      // Live-broadcast the role change so the promoted/demoted user and
      // every other member update without a refetch (see note above).
      getIo().to(roomId).emit('room:updated', { room });
