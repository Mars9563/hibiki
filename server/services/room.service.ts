...
// Promote/demote a participant. Same trust boundary as updateGroup:
// the caller's admin role is checked explicitly on the user-scoped
// client for a clean 403, then the write goes through the service role
// (chat_room_participants has no UPDATE policy for normal users).
export async function setGroupMemberRole({
  ...
  if (membershipError) {
    console.error('setGroupMemberRole membership check error:', membershipError);
    throw new GroupServiceError(500, 'Server error');
  }

  if (!membership || membership.role !== 'admin') {
    throw new GroupServiceError(403, 'Only admins can change member roles');
  }

  const { data: target, error: targetError } = await supabaseSuperUser
    .from('chat_room_participants')
    .select('user_id, role')
    .eq('room_id', roomId)
  ...
  if (targetError) {
    console.error('setGroupMemberRole target lookup error:', targetError);
    throw new GroupServiceError(500, 'Server error');
  }

  if (!target) {
    throw new GroupServiceError(404, 'User is not a member of this group');
  }

  // Already the requested role — nothing to write, just return the
  // current shape so the client stays in sync.
  if (target.role !== role) {
    const { error: updateError } = await supabaseSuperUser
      .from('chat_room_participants')
      .update({ role })
    ...
    if (updateError) {
      console.error('setGroupMemberRole update error:', updateError);
      throw new GroupServiceError(500, 'Failed to update member role');
    }
  }
